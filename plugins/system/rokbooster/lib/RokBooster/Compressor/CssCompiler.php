 ...
 *
 *  See the following:
 *    - entry point for parsing and compiling: RokBooster_Compressor_CssCompiler::parse()
 *    - parsing: RokBooster_Compressor_CssCompiler::parseChunk()
 *    - compiling: RokBooster_Compressor_CssCompiler::compileBlock()
 *
 */
class RokBooster_Compressor_CssCompiler {
	 ...
	 *
	 * Consider the function RokBooster_Compressor_CssCompiler::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by RokBooster_Compressor_CssCompiler::match(), which behaves
	 ...
	 * avoid this, RokBooster_Compressor_CssCompiler::seek() is used to remember and set buffer positions.
	 *
	 ...
	 *
	 * See RokBooster_Compressor_CssCompiler::compileProp()
	 *
	 */
	...
	protected function createChild($fname) {
		$less = new RokBooster_Compressor_CssCompiler(file_get_contents($fname),$this->importDir);
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new RokBooster_Compressor_CssCompiler();
	...
	public static function ccompile($in, $out) {
		if (!is_file($out) || filemtime($in) > filemtime($out)) {
			$less = new RokBooster_Compressor_CssCompiler($in);
	...
	public static function compile($in, $root_url)
	{
		$less = new RokBooster_Compressor_CssCompiler($in, $root_url);
		return $less->parse();
	}
	...
	public static function cexecute($in, $force = false) {
		...
		if ($root !== null) {
			...
			$less = new RokBooster_Compressor_CssCompiler($root);
			$out = array();
			$out['root'] = $root;
