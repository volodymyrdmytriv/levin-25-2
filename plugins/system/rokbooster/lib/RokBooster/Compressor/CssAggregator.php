<?php
/**
 * @version   $Id: CssAggregator.php 7084 2013-02-01 05:24:06Z btowles $
...
class RokBooster_Compressor_CssAggregator
{
				...
				for ($i = 0; $i < $match_count; $i++) {
					if (!empty($matches[1][$i])) {
						...
						if ($import_file != null) {
							...
							$this->buffer   = str_replace($matches[0][$i], $child_contents, $this->buffer);
						}
					}
				}
			}
	...
	{
		$css = new self(@file_get_contents($fname), $this->rootDir);
		return $css;
	}
