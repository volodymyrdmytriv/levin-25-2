 ...
 */
class RokCommon_Form_Exception extends Exception
{
}
 ...
 * @since       11.1
 */
class RokCommon_Form
{


	protected $container;

	const FORM_PATH_PACKAGE = 'rokcommon_formpath';

	const FORM_FIELD_PREFIX = 'RokCommon_Form_Field_';

	const FORM_FIELD_OVERRIDEPATH_PRIORITY = 50;

	const FORM_GROUP_PREFIX = 'RokCommon_Form_Group_';

	const FORM_GROUP_OVERRIDEPATH_PRIORITY = 50;

	const FORM_VALIDATOR_PREFIX = 'RokCommon_Form_Validator_';

	const FORM_VALIDATOR_OVERRIDEPATH_PRIORITY = 50;


	/**
	 * The RokCommon_Registry data store for form fields during display.
	 * @var    object
	 * @since  11.1
	 */
	protected $data;
	...
	/**
	 * @var RokCommon_ClassLoader_PrefixedLoader
	 */
	...
	/**
	 * @var RokCommon_Form_IItemNameHandler
	 */
	...
	public function __construct($name, array $options = array())
	{
		...
		// TODO  move this initialization someplace else   not cool here
		// Force the loaders for the parts to load up
		$this->container->getService('form.field.loader');
		$this->container->getService('form.group.loader');
		$this->container->getService('form.validator.loader');


		// Set the name for the form.
		$this->name = $name;

		// Initialise the RokCommon_Registry data.
	...
	/**
	 * @param \RokCommon_Form_IItemNameHandler $namehandler
	 */
	public function setNamehandler($namehandler)
	{
		$this->namehandler = $namehandler;
	}

	/**
	 * @return \RokCommon_Form_IItemNameHandler
	 */
	public function getNamehandler()
	{
	...
	public function bind($data)
	{
		...
		if (is_object($data)) {
			if ($data instanceof RokCommon_Registry) {
				// Handle a RokCommon_Registry.
				$data = $data->toArray();
	 ...
	 *
	 * @return  array  Array of error messages or RokCommon_Form_Exception objects.
	 *
	 * @since   11.1
	 */
	...
	/**
	 * Method to get a form field represented as a RokCommon_Form_IField object.
	 *
	...
	/**
	 * Method to get an array of RokCommon_Form_AbstractField objects in a given fieldset by name.  If no name is
	 * given then all fields are returned.
	 *
	 * @param   string  $set  The optional name of the fieldset.
	 *
	 * @return  array  The array of RokCommon_Form_IField objects in the fieldset.
	 *
	 * @since   11.1
	 */
		...
		foreach ($sets as $set) {
			...
			if ((string)$set['name']) {
				...
				if (empty($fieldsets[(string)$set])) {
					...
					if (empty($tmp)) {
						$fieldset = (object)array(
							'name'        => (string)$set, 'label'       => '', 'description' => ''
						);
	...
	public function load($data, $replace = true, $xpath = false)
	{
		...
		if (is_string($data)) {
			$data = RokCommon_Utils_XMLHelper::getXML($data, false);
	...
	/**
	 * Proxy for {@link RokCommon_Form_Helper::addFieldPath()}.
	 *
	...
	public static function addGroupPath($new = null)
	{
		$container = RokCommon_Service::getContainer();
		/** @var $loader RokCommon_ClassLoader_PrefixedLoader */
		$loader = $container->getService('form.group.loader');
		$loader->addPath($new, self::FORM_GROUP_PREFIX, self::FORM_GROUP_OVERRIDEPATH_PRIORITY);
	}
