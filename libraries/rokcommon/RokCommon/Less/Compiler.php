<?php
/**
 * @version   $Id: RokCommon_Less_Compiler.class.php 4060 2012-10-02 18:03:24Z btowles $
 ...
 * by `RokCommon_Less_Compiler_Parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `RokCommon_Less_Compiler`. The CSS tree is fed into a
 ...
 *
 * The `parse` function of `RokCommon_Less_Compiler` is the entry point.
 *
 * In summary:
 *
 * The `RokCommon_Less_Compiler` class creates an intstance of the parser, feeds it LESS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `RokCommon_Less_Compiler_Parser` class is only concerned with parsing its input.
 *
 * The `lessc_formatter` takes a CSS tree, and dumps it to a formatted string,
 * handling things like indentation.
 */
class RokCommon_Less_Compiler {
	 ...
	 *
	 * See RokCommon_Less_Compiler::compileProp()
	 *
	 */
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new RokCommon_Less_Compiler_Parser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new RokCommon_Less_Compiler_Parser($this, $name);
	...
	protected function newFormatter() {
		$className = "RokCommon_Less_Compiler_Formatter_Lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "RokCommon_Less_Compiler_Formatter_$this->formatterName";
		}
...
// syntax tree
class RokCommon_Less_Compiler_Parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('RokCommon_Less_Compiler', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = RokCommon_Less_Compiler::preg_quote(self::$commentSingle);
			$commentMultiLeft = RokCommon_Less_Compiler::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = RokCommon_Less_Compiler::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function RokCommon_Less_Compiler::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by RokCommon_Less_Compiler::match(), which behaves
	 ...
	 * avoid this, RokCommon_Less_Compiler::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("RokCommon_Less_Compiler", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = RokCommon_Less_Compiler::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = RokCommon_Less_Compiler::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("RokCommon_Less_Compiler", "preg_quote"), $stop);
	...
	protected function string(&$out) {
		...
		$patt = '([^\n]*?)(@\{|\\\\|' .
			RokCommon_Less_Compiler::preg_quote($delim).')';
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = RokCommon_Less_Compiler::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.RokCommon_Less_Compiler::preg_quote($what), $m, !$until)) return false;
...
class RokCommon_Less_Compiler_Formatter_Classic {
	public $indentChar = "  ";
...
class RokCommon_Less_Compiler_Formatter_Compressed extends RokCommon_Less_Compiler_Formatter_Classic {
...
class RokCommon_Less_Compiler_Formatter_Lessjs extends RokCommon_Less_Compiler_Formatter_Classic {
	public $disableSingle = true;
