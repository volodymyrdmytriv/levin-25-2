...
class plgSystemTooltipsHelper
{
	...
	function onAfterDispatch()
	{
		...
		if (JFactory::getDocument()->getType() !== 'feed' && !JFactory::getApplication()->input->getInt('print') && !JFactory::getApplication()->input->getInt('noscript')) {
			...
			JHtml::script('tooltips/script.js', false, true);
			if ($this->params->load_stylesheet) {
				JHtml::stylesheet('tooltips/style.css', false, true);
			}
	...
	function onAfterRender()
	{
		...
		if (strpos($html, '{' . $this->params->tag) === false) {
			if (!$this->params->hasitems) {
				...
				$html = preg_replace('#\s*<' . 'link [^>]*href="[^"]*/media/tooltips/css/style.css"[^>]* />#s', '', $html);
				$html = preg_replace('#\s*<' . 'script [^>]*src="[^"]*/media/tooltips/js/script[^"]*\.js[^"]*"[^>]*></script>#s', '', $html);
