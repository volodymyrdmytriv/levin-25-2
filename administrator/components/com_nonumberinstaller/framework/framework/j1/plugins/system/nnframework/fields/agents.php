...
class nnFieldAgents
{
	function getInput($name, $id, $value, $params)
	{
		...
		if (empty($groups) || in_array('mobile', $groups)) {
			$agents[] = array(JText::_('ALL'), 'mobile');
		...
		if (empty($groups) || in_array('searchbots', $groups) || in_array('crawlers', $groups)) {
			$agents[] = array(JText::_('ALL'), 'searchbots');
		...
		foreach ($agents as $agent) {
			if (!$agent) {
				...
				$options[] = JHtml::_('select.option', $agent['1'], $agent_name, 'value', 'text');
			}
		}
...
class JElementNN_Agents extends JElement
{
	var $_name = 'Agents';

	function fetchElement($name, $value, &$node, $control_name)
	{
		$this->_nnfield = new nnFieldAgents;
