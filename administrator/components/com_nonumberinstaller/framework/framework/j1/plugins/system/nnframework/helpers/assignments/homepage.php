...
class NNFrameworkAssignmentsHomePage
{
	...
	function passHomePage(&$main, &$params, $selection = array(), $assignment = 'all')
	{
		...
		if ($main->_params->option) {
			...
			if (!isset($home->query['option']) || $home->query['option'] != $main->_params->option) {
				return ($assignment == 'exclude');
			}

			// check set values against home menu query items
			foreach ($home->query as $k => $v) {
				if ((isset($main->_params->{$k}) && $main->_params->{$k} != $v)
					|| (!isset($main->_params->{$k}) && JRequest::getCmd($k) != $v)
				) {
					return ($assignment == 'exclude');
				}
			}
	...
	function checkPass(&$home, $addlang = 0)
	{
		...
		if ($addlang) {
			$sef = $uri->getVar('lang');
			if (empty($sef)) {
				$langs = array_keys(JLanguage::getKnownLanguages(JPATH_SITE));
				foreach ($langs as $lang) {
					$lang = explode('-', $lang);
					$langs[] = array_shift($lang);
				}
