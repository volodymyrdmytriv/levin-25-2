...
Modifications for Iproperty by the Thinkery
*/
...
class ip_MySQL_DB_Backup
{
	...
	{			
		...
		if ($task == MSX_APPEND || $task == MSX_SAVE || $task == MSX_DOWNLOAD)
		{
			...
			{			
				$v = new ipropertyAdmin; // to get version string
                $ipversion = $v->_getversion();

				//$tmp_name = 'ip-'.$v->_getVersion().'-'.date($this->fname_format);
                $tmp_name = 'ipbackup_'.date($this->fname_format).'_v'.$ipversion['iproperty'];
	...
	function _Retrieve($fp, $compress)
	{
		...
		if ($this->comments)
		{
            ...
            $value .= '# IP version: ' .$ipversion['iproperty'] . MSX_NL;
            $value .= '# ' . MSX_NL;
