...
class NSP_GK4_Portal_Mode_5 {
	...
	function output() {
		...
		for($i = 0; $i < count($this->parent->content["ID"]); $i++) {	
			...
			$news_image_content = '<div class="nspImageGallery">' . $news_image . '</div>';
			$news_generated_content = '<div class="nspHeadline">' . $news_header . '</div>';	
		...
		if($this->parent->config["useCSS"] == 1) {
			$document->addStyleSheet( $uri->root().'modules/mod_news_pro_gk4/interface/css/style.portal.mode.5.css', 'text/css' );
		}
		...
		if($this->parent->config['useScript'] == 2) {
			...
			if(array_search($uri->root().'modules/mod_news_pro_gk4/interface/scripts/engine.portal_mode_5.js', $headData_keys) > 0) {
				$engine_founded = true;
			}
			// if mootools file doesn't exists in document head section
			if(!$engine_founded){ 
				// add new script tag connected with mootools from module
				$document->addScript($uri->root().'modules/mod_news_pro_gk4/interface/scripts/engine.portal.mode.5.js');
			}
		}
		//
		require(JModuleHelper::getLayoutPath('mod_news_pro_gk4', 'content.portal.mode.5'));
		require(JModuleHelper::getLayoutPath('mod_news_pro_gk4', 'default.portal.mode.5'));
	}
}
//
function Portal_Mode_5_getData($parent) {
	$db =& JFactory::getDBO();
	...
	if( $parent->config["data_source"] == "com_categories" ||
		...
		// getting content
		$amountOfArts = $parent->config['news_portal_mode_5_amount'];
		...
		// getting insance of VM source class
	    $newsClass = new NSP_GK4_VM_Source();
		// Getting list of categories
		$categories = $newsClass->getSources($parent->config);
		// getting content
		$amountOfProducts = $parent->config['news_portal_mode_5_amount']; 
