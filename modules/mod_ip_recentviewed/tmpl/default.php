...
?>
<div style="position: relative;" class="ip_recentmodviewed_holder<?php echo $moduleclass_sfx; ?>">
    <table width="100%" cellpadding="5" cellspacing="1" class="ip_recentviewedmod_table">
        <?php
           if( $params->get('iplayout') == 1 ): //horizontal layout
                for($i = 0; $i < sizeof($list); $i++):
                    ...
                    echo '<tr>
                             <td width="10%" valign="top">
                               <div class="ip_recentviewed_thumb" style="position: relative; width: '.$thumb_width.' !important; height: '.$thumb_height.' !important; border: solid 1px '.$params->get( 'border_color', '#fff' ).'; overflow:hidden !important;">
                                    '.$list[$i]->mainimage.$banner_display.'
                               </div>
                             </td>
                             <td width="90%" valign="top">
                               <div class="ip_recentviewed_overview" style="margin-top: 5px;">
                                    <a href="' . $list[$i]->link . '" class="ip_recentviewed_title">' . $list[$i]->street_address . '</a> - ';
            ...
            <tr>
                ...
                for($i = 0; $i < count($list); $i++):
                    ...
                    echo '<td width="'.$percentage.'%" valign="top">
                               ...
                               <div class="ip_recentviewed_overview" style="margin-top: 5px;">';
                                    echo '<p>';
                                    echo '<a href="' . $list[$i]->link . '" class="ip_recentviewed_title">' . $list[$i]->street_address . '</a><br />';
                                    echo '<em>';
                                    if($list[$i]->city) echo $list[$i]->city;
                                    if($list[$i]->state) echo ', '.$list[$i]->state;
                                    if($list[$i]->province) echo ', '.$list[$i]->province;
