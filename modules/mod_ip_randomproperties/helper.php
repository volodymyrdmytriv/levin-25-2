...
class modIPRandomHelper
{
	...
	function getList(&$params)
	{
        ...
        $rows = modIPRandomHelper::getPropertiesList($where,0,$count, $sort, $order);
        ...
        if( $rows ){
            foreach ( $rows as $row )
            {
                ...
                $lists[$i]->mainimage       = ipropertyHTML::getThumbnail($row->id, $lists[$i]->link, $row->street_address, $thumb_width, ' class="ip_random_img"', '', $usethumb);
                ...
                $prepared_text = $row->short_description ? modIPRandomHelper::prepareContent($row->short_description, $params->get('preview_count', 250)) : modIPRandomHelper::prepareContent($row->description, $params->get('preview_count', 250));
                ...
                $lists[$i]->introtext .= ' <a href="' . $lists[$i]->link . '">' . JTEXT::_('MOD_IP_RANDOM_READ_MORE') . '</a>';
