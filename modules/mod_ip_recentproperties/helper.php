...
class modIPRecentHelper
{
	...
	function getList(&$params)
	{
        ...
        $where = array();
        $where[] = 'p.created >= DATE_SUB(curdate(), interval ' . $params->get('history', 1) . ' ' . $params->get('interval', 'week') . ')';
        ...
        $rows = modIPRecentHelper::getPropertiesList($where,0,$count, $sort, $order);
        ...
        if( $rows ){
            foreach ( $rows as $row )
            {
                ...
                $prepared_text = $row->short_description ? modIPRecentHelper::prepareContent($row->short_description, $params->get('preview_count', 250)) : modIPRecentHelper::prepareContent($row->description, $params->get('preview_count', 250));
                ...
                $lists[$i]->introtext .= ' <a href="' . $lists[$i]->link . '">' . JTEXT::_('MOD_IP_RECENT_READ_MORE') . '</a>';
