...
class modIPRecentsoldHelper
{
    ...
    function getPropertiesList( $where, $limitstart=0, $limit=9999, $sort = 'p.modified', $order = 'DESC' )
	{
		$db = JFactory::getDBO();
	...
	function getList(&$params)
	{
        ...
        $where  = array();
        $where[] = 'p.stype IN ('.$stype.')';
        if($category){
            $where[] = 'pm.cat_id IN ('.implode(',', $category).')';
        }

        $rows = modIPRecentsoldHelper::getPropertiesList($where,0,$count, $sort, $order);
        ...
        if( $rows ){
            foreach ( $rows as $row )
            {
                ...
                $prepared_text = $row->short_description ? modIPRecentsoldHelper::prepareContent($row->short_description, $params->get('preview_count', 250)) : modIPRecentsoldHelper::prepareContent($row->description, $params->get('preview_count', 250));
                ...
                $lists[$i]->introtext .= ' <a href="' . $lists[$i]->link . '">' . JTEXT::_('MOD_IP_RECENT_SOLD_READ_MORE') . '</a>';
