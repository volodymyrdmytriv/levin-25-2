...
function evolve_discussion_rss() {
  ...
  $text = "<span class=\"comment-feed-link\"><a title=\"Follow replies\" class=\"tipsytext follow-replies\" href=\"{$uri}\"><i class='fa fa-rss'></i></a></span>";
	echo $text;
}
...
function evolve_comment_author( $meta_format = '%avatar% %name%' ) {
	...
	if ( strpos( $meta_format, '%' ) === false ) {
		...
		$open .= '<div class="comment-header"><div class="comment-author vcard">' . "\n";
		$close  = "\n" . '<!--END .comment-author-->' . "\n";
		$close .= '</div>' . "\n";
