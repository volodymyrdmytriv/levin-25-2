...
class RokSprocket_Provider_Pods_Filter_Type_Article extends RokSprocket_Provider_Filter_Type_AbstractArticle
{
	/**
	 * @var string
	 */
	...
	protected function getInput($name = RokCommon_Filter_Type::JAVASCRIPT_NAME_VARIABLE, $value = '')
	{
        ...
        $link = get_bloginfo('wpurl').'/wp-admin/admin-ajax.php?action=roksprocket_postlist&nonce='.$nonce.'&provider=pods&TB_iframe=true&height=425&width=555&modal=false';

        $title = $wpdb->get_var('SELECT name FROM '.$wpdb->prefix . 'pod WHERE id = ' . (int)$value);
