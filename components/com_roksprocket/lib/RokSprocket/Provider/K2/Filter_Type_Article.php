...
class RokSprocket_Provider_K2_Filter_Type_Article extends RokSprocket_Provider_Filter_Type_AbstractArticle
{
	/**
	 * @var string
	 */
	...
	{
		return rc__('ROKSPROCKET_FILTER_K2_ARTICLE_RENDER', $this->getTypeDescription($this->getChunkType()));
	}
	...
	public function render($name, $type, $values)
	{
		...
		return rc__('ROKSPROCKET_FILTER_K2_ARTICLE_RENDER', $this->getInput($name, $value));
	}
	...
	protected function getInput($name = RokCommon_Filter_Type::JAVASCRIPT_NAME_VARIABLE, $value = '')
	{
		...
		$html = array();
		$link = 'index.php?option=com_roksprocket&amp;view=k2items&amp;layout=model&amp;tmpl=component&amp;function=jSelectUser_Sprocket';

		$db = JFactory::getDBO();
		$db->setQuery('SELECT title' . ' FROM #__k2_items' . ' WHERE id = ' . (int)$value);
		$title = $db->loadResult();
		...
		if (empty($title)) {
			$title = JText::_('COM_K2_CHANGE_ITEM');
		}
		...
		$html[] = '	  <a class="modal" title="' . JText::_('COM_K2_CHANGE_ITEM') . '"' . ' href="' . $link . '&amp;' . JSession::getFormToken() . '=1"' . ' rel="{handler: \'iframe\', size: {x: 900, y: 500}}">';
