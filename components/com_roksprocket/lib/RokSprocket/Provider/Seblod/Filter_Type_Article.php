...
class RokSprocket_Provider_Seblod_Filter_Type_Article extends RokSprocket_Provider_Filter_Type_AbstractArticle
{
	/**
	 * @var string
	 */
	...
	{
		return rc__('ROKSPROCKET_FILTER_SEBLOD_ARTICLE_RENDER', $this->getTypeDescription($this->getChunkType()));
	}
	...
	public function render($name, $type, $values)
	{
		...
		return rc__('ROKSPROCKET_FILTER_SEBLOD_ARTICLE_RENDER', $this->getInput($name, $value));
	}
	...
	protected function getInput($name = RokCommon_Filter_Type::JAVASCRIPT_NAME_VARIABLE, $value = '')
	{
		...
		$html = array();
        $link = 'index.php?option=com_roksprocket&amp;view=sebloditems&amp;layout=model&amp;tmpl=component&amp;function=jSelectUser_Sprocket';
