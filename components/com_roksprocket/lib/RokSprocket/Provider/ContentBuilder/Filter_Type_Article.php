<?php
/**
 * @version	$Id: Filter_Type_Article.php 59614 2013-04-18 15:15:36Z steph $
 * @author	 RocketTheme http://www.rockettheme.com
 * @copyright Copyright (C) 2007 - 2013 RocketTheme, LLC
 * @license	http://www.gnu.org/licenses/gpl-2.0.html GNU/GPLv2 only
 */

class RokSprocket_Provider_ContentBuilder_Filter_Type_Article extends RokSprocket_Provider_Filter_Type_AbstractArticle
{
	/**
	 * @var string
	 */
	...
	{
		return rc__('ROKSPROCKET_FILTER_CONTENTBUILDER_ARTICLE_RENDER', $this->getTypeDescription($this->getChunkType()));
	}
	...
	public function render($name, $type, $values)
	{
		...
		return rc__('ROKSPROCKET_FILTER_CONTENTBUILDER_ARTICLE_RENDER', $this->getInput($name, $value));
	}
	...
	protected function getInput($name = RokCommon_Filter_Type::JAVASCRIPT_NAME_VARIABLE, $value = '')
	{
		...
		$html = array();
		$link = 'index.php?option=com_roksprocket&amp;view=contentbuilderitems&amp;layout=model&amp;tmpl=component&amp;function=jSelectUser_Sprocket';

		$db = JFactory::getDBO();
        $db->setQuery('SELECT title' . ' FROM #__content' . ' WHERE id = ' . (int)$value);
		$title = $db->loadResult();
		...
		if (empty($title)) {
			$title = JText::_('COM_CONTENTBUILDER_CHANGE_ITEM');
		}
		...
		$html[] = '	  <a class="modal" title="' . JText::_('COM_CONTENTBUILDER_CHANGE_ITEM') . '"' . ' href="' . $link . '&amp;' . JSession::getFormToken() . '=1"' . ' rel="{handler: \'iframe\', size: {x: 900, y: 500}}">';
